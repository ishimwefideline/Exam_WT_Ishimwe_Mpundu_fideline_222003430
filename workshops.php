...
<form action="" method="POST" onsubmit="return confirmInsert();">
    <h3 style="font-size: 20px;color: deeppink;"><i>WORKSHOP FORM</i></h3>
    <label for="WorkshopID">WorkshopID:</label>
    <input type="number" id="WorkshopID" name="WorkshopID"><br><br>
...
if(isset($_POST['send'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO workshops (WorkshopID, Title, Duration, InstructorID, Location) VALUES (?, ?, ?, ?, ?)");

    $stmt->bind_param("issss", $WorkshopID, $Title, $Duration, $InstructorID, $Location);
