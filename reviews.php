...
<form action="" method="POST" onsubmit="return confirmInsert();">
    <h3 style="font-size: 20px;color: deeppink;"><i>REVIEW FORM</i></h3>
    <label for="ReviewID">ReviewID:</label>
    <input type="number" id="ReviewID" name="ReviewID"><br><br>
...
if(isset($_POST['send'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO reviews (ReviewID, WorkshopID, AttendeeID, Comment, ReviewDate) VALUES (?, ?, ?, ?,?)");

    $stmt->bind_param("issss", $ReviewID, $WorkshopID, $AttendeeID, $Comment, $ReviewDate);
