...
<form action="" method="POST" onsubmit="return confirmInsert();">
    <h3 style="font-size: 20px;color: deeppink;"><i>RESOURCES FORM</i></h3>
...
if(isset($_POST['send'])) {
    ...
    $WorkshopID = isset($_POST['WorkshopID']) ? $_POST['WorkshopID'] : '';
    $UploadedBy = isset($_POST['UploadedBy']) ? $_POST['UploadedBy'] : '';
    $UploadDate = isset($_POST['UploadDate']) ? $_POST['UploadDate'] : '';

    // Insert new record into the database
    $stmt = $connection->prepare("INSERT INTO resources (ResourceID, Title, Description, WorkshopID, UploadedBy, UploadDate) VALUES (?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("ississ", $ResourceID, $Title, $Description, $WorkshopID, $UploadedBy, $UploadDate);
