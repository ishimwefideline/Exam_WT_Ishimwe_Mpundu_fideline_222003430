...
<form action="" method="POST" onsubmit="return confirmInsert();">
    <h3 style="font-size: 20px;color: deeppink;"><i>INSTRUCTORS FORM</i></h3>
    <label for="InstructorID">InstructorID:</label>
    <input type="number" id="InstructorID" name="InstructorID"><br><br>
...
if(isset($_POST['send'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO instructors (InstructorID, UserID, Fullname, Specialization) VALUES (?, ?, ?, ?)");

    $stmt->bind_param("isss", $InstructorID, $UserID, $Fullname, $Specialization);
